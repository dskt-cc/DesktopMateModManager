...
import { MOD_CATEGORIES, SORT_OPTIONS } from "../constants/modFilters";
import { modTypeButtons } from "../components/ModTypeButtons";
import { GetAvailableMods } from "../../wailsjs/go/main/App";
...
export function Mods() {
    ...
    useEffect(() => {
        const loadMods = async () => {
            try {
                const modsData = await GetAvailableMods();
                setMods(modsData);
            } catch (err) {
    ...
    const getSortedMods = (modsToSort) => {
        switch (sortBy) {
            case "downloads":
            ...
            case "featured":
                return [...modsToSort].sort((a, b) => (b.featured ? 1 : 0) - (a.featured ? 1 : 0));
            case "popular":
            default:
                return [...modsToSort].sort((a, b) => b.downloads * 2 + b.views - (a.downloads * 2 + a.views));
        }
    };
    ...
    const getFilteredMods = () => {
        ...
        if (selectedType !== "All") {
            filteredMods = filteredMods.filter((mod) => mod.meta.type === selectedType);
        }

        if (selectedCategory) {
            filteredMods = filteredMods.filter((mod) => mod.meta.category.includes(selectedCategory));
        }
                ...
                <div className="flex flex-col gap-6 sm:gap-8 mb-8 sm:mb-12">
                    ...
                    <div className="bg-miku-deep/10 backdrop-blur-sm border border-miku-deep/20 rounded-xl p-4 sm:p-6">
                        ...
                        <div className="mt-4 sm:mt-6 flex flex-wrap items-center gap-2">
                            <span className="text-sm text-miku-light/70">Active Filters:</span>
                            {selectedType !== "All" && (
                  ...
                  {selectedType}
                            ...
                            {selectedCategory && (
                                <span className="px-3 py-1 text-sm bg-miku-teal/20 text-miku-teal rounded-full">
                  {MOD_CATEGORIES.find((c) => c.value === selectedCategory)?.label}
                </span>
                            )}
                            {searchQuery && (
                                <span className="px-3 py-1 text-sm bg-miku-teal/20 text-miku-teal rounded-full">
